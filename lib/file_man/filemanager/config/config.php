...
// установка разрешенных директорий
$allowed = [
    'images', 
    'load/training'
];

$dir_base = BaseDir('images', $allowed);
...
if (!file_exists($config['current_path'])) {
    mkdir($config['current_path']);
}


function BaseDir($dir_base = 'images', $allowed = ['images', 'load/training']){

    $f_path = 
        (isset($_POST['f_path']) && !empty($_POST['f_path']))
        ? $_POST['f_path']
        : ((isset($_GET['f_path']) && !empty($_GET['f_path']))
            ? $_GET['f_path']
            : false);

    if($f_path){

        if (substr($f_path, 0, 1) == '/')
            $f_path = substr($f_path, 1);

        if (substr($f_path, -1) == '/')
            $f_path = substr($f_path, 0, -1);

        if (in_array($f_path, $allowed))
            $dir_base = $f_path;
            ...
            foreach ($allowed as $allowed_dir) {
                ...
                $allowed_dir = explode("/", $allowed_dir);
                $f_path_ = explode("/", $f_path);
                
                for ($i=0; $i < count($allowed_dir); $i++) 

                    if($allowed_dir[$i] !== $f_path_[$i])
                        $alw = false;
